...
export default function TeacherCodesTab() {
    ...
    const assignTeacherCode = (teacherCodeId, userId) => {
        Api.put('/teacher-code', { data: { user_id: userId, teacher_code_id: teacherCodeId } }).then(({ data }) => {
            const { teacher_codes } = data;
            setTeacherCodes(teacher_codes);
        });
    ...
    return (
        <Item className='bordered'>
            <div className="title">
                ...
                <Button variant="contained" onClick={handleGenerateTeacherCode} startIcon={<Add></Add>}>Générer un code enseignant</Button>
            </div>
            <TeacherCodeGrid rows={teacherCodes} refresh={getTeacherCodes} deleteTeacherCode={deleteTeacherCode} assignTeacherCode={assignTeacherCode} errorTeacherCodeHandler={handleErrorTeacherCode} />
            ...
            <Snackbar onClose={handleCloseErrorMessage} open={showErrorMessage} autoHideDuration={3000}>
                <Alert onClose={handleCloseErrorMessage} severity="error" sx={{ width: '100%' }} md={{ minWidth: '300px' }}>
                    {errorMessage}
                </Alert>
            </Snackbar>
        </Item>
